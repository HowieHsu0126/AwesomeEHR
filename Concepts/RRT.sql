...
, combined_data AS (
    SELECT
      stg0.stay_id,
      (EXTRACT(EPOCH FROM (mv.endtime - mv.starttime))/3600) AS rrt_hour,
    ...
    FROM stg0
    INNER JOIN mv_ranges mv ON stg0.stay_id = mv.stay_id
    INNER JOIN icu_aki aki ON aki.stay_id = stg0.stay_id
    WHERE stg0.charttime >= mv.starttime AND stg0.charttime <= mv.endtime
  )
  SELECT
    stay_id,
    ROUND(AVG(rrt_hour), 2) AS total_rrt_duration,
    MAX(dialysis_present) AS dialysis_present,
    MAX(dialysis_active) AS dialysis_active,
    MAX(dialysis_type) AS most_common_dialysis_type
  FROM combined_data
  GROUP BY stay_id
) TO '/home/hwxu/Projects/Dataset/PKU/AMIA/wentie_rrt.csv' WITH CSV HEADER;