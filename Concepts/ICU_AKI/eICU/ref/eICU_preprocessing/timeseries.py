...
def reconfigure_timeseries(timeseries, offset_column, feature_column=None, test=False):
    ...
    timeseries.set_index(['patientunitstayid', pd.to_timedelta(
        timeseries[offset_column], unit='T')], inplace=True)
...
def resample_and_mask(timeseries, eICU_path, header, mask_decay=True, decay_rate=4/3, test=False,
    ...
    # save to csv
    if test is False:
        final.to_csv(eICU_path + 'preprocessed_timeseries.csv',
...
def gen_timeseries_file(eICU_path, test=False):
    ...
    timeseries_resp['respchartnumeric'] = [encode_value(
        value) for value in timeseries_resp.respchartvalue.values]
    ...
    merged = pd.merge(timeseries_lab, timeseries_resp,
                      how='outer', on=['patient', 'time'])
    merged = pd.merge(merged, timeseries_periodic,
                      how='outer', on=['patient', 'time'])
    merged = pd.merge(merged, timeseries_aperiodic, how='outer', on=[
                      'patient', 'time'], sort=True)
...
def add_time_of_day(processed_timeseries, flat_features):
    ...
    processed_timeseries['hour'] = processed_timeseries['time'] + \
...
def further_processing(eICU_path, test=False):
    ...
    processed_timeseries = pd.read_csv(
        eICU_path + 'preprocessed_timeseries.csv')
    ...
    flat_features.rename(
        columns={'patientunitstayid': 'patient'}, inplace=True)
    ...
    mean_masks = processed_timeseries[mask_cols].eq(
        1).groupby('patient').mean().mean()
    mask_to_flat = list(mean_masks[(mean_masks <= 2 / 24)].index)
    cols_to_flat = [x[:-5]
                    for x in mask_to_flat] + mask_to_flat  # remove '_mask'

    # keep only the most recent measurement, and it's corresponding mask value
    flat_features = flat_features.join(
        processed_timeseries.loc[processed_timeseries['time']
                                 == 24][cols_to_flat],
    ...
    processed_timeseries.drop(
        columns=['RR (patient)', 'RR (patient)_mask'], inplace=True)
