...
create materialized view commonlabs as
  select labsbefore24h.labname, count(distinct la.patientunitstayid) as count
    ...
    from (select labname, patientunitstayid from eicu_crd.lab where labresultoffset between -1440 and 1440) as labsbefore24h
...
create materialized view commonresp as
  select respbefore24h.respchartvaluelabel, count(distinct la.patientunitstayid) as count
    ...
    from (select respchartvaluelabel, patientunitstayid from eicu_crd.respiratorycharting where respchartoffset between -1440 and 1440) as respbefore24h
    inner join labels as la
