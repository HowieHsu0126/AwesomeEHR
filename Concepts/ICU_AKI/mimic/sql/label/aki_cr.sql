...
WITH cr AS (
    ...
    SELECT
        ...
        ROW_NUMBER() OVER (PARTITION BY ie.subject_id,
            ie.hadm_id ORDER BY le.charttime) AS rn
    FROM
        mimiciv_icu.icustays ie
        LEFT JOIN mimiciv_hosp.labevents le ON ie.subject_id = le.subject_id
...
cr_baseline AS (
    ...
    FROM (
        ...
        FROM
            cr
    WHERE
        charttime >= DATETIME_SUB(intime, INTERVAL '7' DAY) -- 基线时间窗口为 ICU 入院前 7 天
        AND charttime <= intime -- 确保是进入ICU7天内的肌酐数据
) sub
    WHERE
        rn = 1 -- 选择最小肌酐值及其对应时间
),
...
(cr2.creat - cr1.creat) AS creat_diff,
    ...
    FROM
        cr cr1
        JOIN cr cr2 ON cr1.subject_id = cr2.subject_id
        ...
        JOIN cr_baseline rb ON cr1.hadm_id = rb.hadm_id -- 引入基线数据进行对比
            AND cr1.stay_id = rb.stay_id
    WHERE
        cr2.creat >= 1.5 * rb.baseline_creat -- 肌酐值超过基线的 1.5 倍时诊断为 AKI
        -- 确保肌酐时间与基线时间的差距在7天以内
        AND cr2.charttime <= rb.baseline_time + INTERVAL '7' DAY
),
aki_diagnosis AS (
    -- 确定 AKI 诊断的时间，7 天内升高超过阈值时诊断为 AKI
    SELECT
        rb.hadm_id,
        rb.stay_id,
        rb.baseline_time,
        rb.baseline_creat,
        aki_7.aki_7day_time,
        CASE WHEN aki_7.aki_7day_time IS NOT NULL THEN
            'AKI-Cr within 7day'
        ELSE
            'No AKI'
        END AS aki_status,
    ...
    FROM
        cr_baseline rb
        LEFT JOIN aki_7day aki_7 ON rb.hadm_id = aki_7.hadm_id
