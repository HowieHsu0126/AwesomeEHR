...
-- 结合肌酐、尿量、RRT 和 AKI 状态
WITH combined_aki AS (
    SELECT
        cr.hadm_id,
        cr.stay_id,
        cr.subject_id,  -- 使用 subject_id 代替 stay_id
        ...
        crrt.charttime AS rrt_charttime -- 记录 RRT 的时间戳
    FROM
        aki_cr cr
...
earliest_aki AS (
    SELECT
        hadm_id,
        subject_id,  -- 使用 subject_id 代替 stay_id
...
SELECT DISTINCT
    subject_id,  -- 使用 subject_id 代替 stay_id
    earliest_time AS aki_timepoint
FROM
    final_aki
WHERE
